...
/**
 * all changes here - namespace naming
 */
namespace typer {
    template<typename T>
    struct Ok {
...
typer::Ok<CleanT> Ok(T&& val) {
    return typer::Ok<CleanT>(std::forward<T>(val));
}

inline typer::Ok<void> Ok() {
    return typer::Ok<void>();
}

template<typename E, typename CleanE = typename std::decay<E>::type>
typer::Err<CleanE> Err(E&& val) {
    return typer::Err<CleanE>(std::forward<E>(val));
}
...
struct Map<Ret (Arg)> {
    ...
    static Result<Ret, E> map(const Result<T, E>& result, Func func) {
        ...
        if (result.isOk()) {
            ...
            return typer::Ok<Ret>(std::move(res));
        }

        return typer::Err<E>(result.storage().template get<E>());
    }
};
...
struct Map<Ret (void)> {
    ...
    static Result<Ret, E> map(const Result<T, E>& result, Func func) {
        ...
        if (result.isOk()) {
            auto ret = func();
            return typer::Ok<Ret>(std::move(ret));
        }
...
struct Map<Ret (Cls::*)(Arg) const> {
    ...
    static Result<T, Ret> map(const Result<T, E>& result, Func func) {
        if (result.isErr()) {
            ...
            return typer::Err<Ret>(res);
        }

        return typer::Ok<T>(result.storage().template get<T>());
    }
...
struct Storage {
    ...
    void construct(typer::Ok<T> ok)
    {
        new (&storage_) T(ok.val);
        initialized_ = true;
    }
    void construct(typer::Err<E> err)
    {
...
struct Result {
    ...
    Result(typer::Ok<T> ok)
        : ok_(true)
    {
        storage_.construct(std::move(ok));
    }

    Result(typer::Err<E> err)
        : ok_(false)
    {
...
template<typename T, typename E>
bool operator==(const Result<T, E>& lhs, typer::Ok<T> ok) {
    static_assert(concept::EqualityComparable<T>::value, "T must be EqualityComparable for Result to be comparable");
...
template<typename E>
bool operator==(const Result<void, E>& lhs, typer::Ok<void>) {
    return lhs.isOk();
}

template<typename T, typename E>
bool operator==(const Result<T, E>& lhs, typer::Err<E> err) {
    static_assert(concept::EqualityComparable<E>::value, "E must be EqualityComparable for Result to be comparable");
